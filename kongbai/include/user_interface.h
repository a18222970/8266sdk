...
/* user task's prio must be 0/1/2 !!!  用户任务的优先级必须是0/1/2*/
enum {
    USER_TASK_PRIO_0 = 0,
    USER_TASK_PRIO_1,
    USER_TASK_PRIO_2,
...
enum flash_size_map {
    FLASH_SIZE_4M_MAP_256_256 = 0,  /**<  Flash size  Flash大小 : 4Mbits. Map : 256KBytes + 256KBytes */
...
typedef enum {
    WIFI_SCAN_TYPE_ACTIVE = 0,  /**< active scan  主动扫描 */
    WIFI_SCAN_TYPE_PASSIVE,     /**< passive scan  被动扫描*/
} wifi_scan_type_t;

/** @brief Range of active scan times per channel  每个通道的有效扫描时间的短范围*/
typedef struct {
    uint32_t min;  /**< minimum active scan time per channel, units: millisecond 每个通道的最小活动扫描时间，单位:毫秒*/
    uint32_t max;  /**< maximum active scan time per channel, units: millisecond, values above 1500ms may
                                          cause station to disconnect from AP and are not recommended.
                                          每个通道的最大活动扫描时间，单位:毫秒，大于1500ms的值可能导致工作站与AP断开，不建议这样做。 */
} wifi_active_scan_time_t;

/** @brief Aggregate of active & passive scan time per channel 每个通道的主动和被动扫描时间的总和*/
typedef union {
...
struct scan_config {
    uint8 *ssid;    // Note: ssid == NULL, don't filter ssid. 注意:ssid == NULL，不要过滤ssid。
    uint8 *bssid;    // Note: bssid == NULL, don't filter bssid. 注意:bssid == NULL，不要过滤bssid。
    uint8 channel;    // Note: channel == 0, scan all channels, otherwise scan set channel. 注:通道== 0，扫描所有通道，否则扫描集通道。
    uint8 show_hidden;    // Note: show_hidden == 1, can get hidden ssid routers' info. 注意:show_hidden == 1，可以获取隐藏的ssid路由器信息。
    wifi_scan_type_t scan_type; // scan type, active or passive  扫描类型，主动或被动
    wifi_scan_time_t scan_time; // scan time per channel  每通道扫描时间
};
 ...
 * 得到2个u32_t值(纠正溢出)之间的绝对差值“a”应该比“b”“更高”(没有溢出)。*/
...
typedef struct {
    ...
    uint32_t addr;            /* the partition address 分区地址*/
    uint32_t size;            /* the partition size 分区大小*/
} partition_item_t;
